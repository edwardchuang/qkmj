#ifndef MJGPS_LOGGER_H
#define MJGPS_LOGGER_H

#include <stdio.h>
#include <stdarg.h>

typedef enum {
    LOG_LEVEL_DEBUG = 0,
    LOG_LEVEL_INFO,
    LOG_LEVEL_WARN,
    LOG_LEVEL_ERROR,
    LOG_LEVEL_FATAL
} LogLevel;

/**
 * @brief Logs a message to the console and optionally to MongoDB.
 * 
 * @param level Severity level of the log.
 * @param file Source file name (__FILE__).
 * @param line Source line number (__LINE__).
 * @param fmt Printf-style format string.
 * @param ... Arguments for format string.
 */
void log_message(LogLevel level, const char *file, int line, const char *fmt, ...);

/**
 * @brief Logs a game record to MongoDB with level="game".
 * 
 * @param json_str JSON string representing the game record.
 */
void log_game_record(const char *json_str);

// Macros to automatically capture file and line
#define LOG_DEBUG(...) log_message(LOG_LEVEL_DEBUG, __FILE__, __LINE__, __VA_ARGS__)
#define LOG_INFO(...)  log_message(LOG_LEVEL_INFO,  __FILE__, __LINE__, __VA_ARGS__)
#define LOG_WARN(...)  log_message(LOG_LEVEL_WARN,  __FILE__, __LINE__, __VA_ARGS__)
#define LOG_ERROR(...) log_message(LOG_LEVEL_ERROR, __FILE__, __LINE__, __VA_ARGS__)
#define LOG_FATAL(...) log_message(LOG_LEVEL_FATAL, __FILE__, __LINE__, __VA_ARGS__)

#endif
