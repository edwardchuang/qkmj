  ...
  while (Check_for_data(0)) {
    ...
    if (input_mode == PLAY_MODE) {
      ...
      switch (key) {
        ...
        case ',':
        case KEY_LEFT:
          if (play_mode != THROW_CARD) break;
          current_item--;
          if (current_item == -1) current_item = pool[my_sit].num;
          pos_x = INDEX_X + (16 - pool[my_sit].num + current_item) * 2;
          if (current_item == pool[my_sit].num) pos_x++;
          return_cursor();
          break;
        case '.':
        case KEY_RIGHT:
          if (play_mode != THROW_CARD) break;
          current_item++;
          if (current_item == pool[my_sit].num + 1) current_item = 0;
          pos_x = INDEX_X + (16 - pool[my_sit].num + current_item) * 2;
          if (current_item == pool[my_sit].num) pos_x++;
          return_cursor();
          break;
        case '`':
          ...
          if (play_mode == THROW_CARD) {
            if (in_join) {
              ...
              card_index--;
              if (card_index < card_point) card_index = 143;
              card = mj[card_index];
      ...
      switch (key) {
        ...
        case KEY_LEFT:
          current_check--;
          if (current_check == -1) current_check = check_number - 1;
          check_x = org_check_x + current_check * 3;
          return_cursor();
          break;
        case '.':
        case KEY_RIGHT:
          current_check++;
          if (current_check == check_number) current_check = 0;
          check_x = org_check_x + current_check * 3;
          return_cursor();
          break;
        case KEY_ENTER:
    ...
    /* ---------  TALK_MODE  ------------ */
      ...
      switch (key) {
        ...
        case ENTER:  // TALK TO Others
          switch (input_mode) {
            case TALK_MODE:
              if (talk_buf_count == 0) break;
              strncpy(history[h_tail], talk_buf, sizeof(history[h_tail]) - 1);
              history[h_tail][sizeof(history[h_tail]) - 1] = '\0';
              command_parser(talk_buf);
              h_tail = (h_tail + 1) % HISTORY_SIZE;
              history[h_tail][0] = 0;
              if (h_tail == h_head) h_head = (h_head + 1) % HISTORY_SIZE;
              h_point = h_tail;
              clear_input_line();
              talk_x = 0;
              return_cursor();
