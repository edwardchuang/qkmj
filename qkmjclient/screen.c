...
void printstr(WINDOW* win, char* str) {
  int i, len;
  len = strlen(str);
  for (i = 0; i < len; i++) printch(win, str[i]);
}
...
void clear_screen_area(int ymin, int xmin, int height, int width) {
  ...
  line_buf[width] = '\0';
  for (i = 0; i < height; i++) wmvaddstr(stdscr, ymin + i, xmin, line_buf);
  wrefresh(stdscr);
}
...
void ask_question(char* question, char* answer, int ans_len, int type) {
  ...
  wrefresh(inputwin);
  mvwgetstring(inputwin, 0, strlen(question), ans_len, (unsigned char*)answer,
               type);
  werase(inputwin);
...
void init_playing_screen() {
  ...
  comment_y = 19;
  commentwin =
      newwin(comment_bottom - comment_up + 1, comment_right - comment_left + 1,
...
/* wmvaddstr implementation moved to misc.c or defined here as macro wrapper if
 * not linked? */
/* Actually I defined wmvaddstr in misc.c.
   ...
   So I should remove it here if it's in misc.c, or keep it here and remove from
   misc.c. Since screen.c handles screen drawing, it belongs here. But misc.c
   used it. I added it to misc.c previously. If I keep it here, I should delete
   from misc.c or rename. Let's look at misc.c again. It had wmvaddstr wrapper.
